...
- Devnet program ID: `DaAcmKvC3PLL4avmjLnfF2uNuYKaFjNYmmhRKYiXbqWV`
- IDL: [`shared/src/solana/idl/basira.json`](../shared/src/solana/idl/basira.json) (+ generated TS types in `basira.ts`, program ID constant in `program-id.ts`)
- Upgrade authority: deployer keypair (will move to multisig pre-mainnet)

The instruction signatures and IDL are frozen. Breaking changes ship as explicit version bumps.
...
# Re-sync the IDL into shared/
cp target/idl/basira.json ../shared/src/solana/idl/basira.json
cp target/types/basira.ts ../shared/src/solana/idl/basira.ts
```

A clean redeploy (program upgrade + IDL upload) costs ~4.2 SOL on devnet; most of that is the temporary buffer account, which is refunded after the upgrade completes.
